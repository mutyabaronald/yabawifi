...
function SuperDashboard() {
  ...
  const [alertsSettings, setAlertsSettings] = useState({
    emails: [],
    thresholds: { downHotspotMinutes: 30 },
  });
    ...
    async function load() {
      try {
        ...
        const [
          ...
          fetch("/api/super/contact-info/manage", {
            headers: authHeaders,
          }).catch(() => null),
        ...
        setWallet({
          balance: walletJson?.balance || 0,
          transactions: walletJson?.transactions || [],
        });
        ...
        try {
          ...
          if (settingsJson) {
            if (typeof settingsJson.commissionRate === "number")
              setCommissionRate(settingsJson.commissionRate);
            if (Array.isArray(settingsJson.defaultPackages))
              setDefaultPackages(settingsJson.defaultPackages);
            if (settingsJson.businessRules)
              setBusinessRules(settingsJson.businessRules);
          }
        } catch {}
        ...
        try {
          ...
          if (contactJson?.contactInfo) {
            setContactInfo({
              ...
              whatsapp:
                contactJson.contactInfo.whatsapp || contactInfo.whatsapp,
  ...
  const performSearch = async (query) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/super/search?q=${encodeURIComponent(query)}`,
        { headers },
      );
  ...
  const loadOwnerDetails = async (ownerId) => {
    try {
      const token = localStorage.getItem("superToken");
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      const res = await fetch(`/api/super/owners/${ownerId}/details`, {
        headers,
      });
      const data = await res.json();
              ...
              >
                <div
                  style={{
                    ...
                    background: "var(--accent)",
                    borderRadius: 999,
                    width: `${Math.max(0, Math.min(metrics.ownerSignups % 100, 100))}%`,
                  }}
                />
              </div>
            </div>
              ...
              >
                <div
                  style={{
                    ...
                    background: "var(--accent)",
                    borderRadius: 999,
                    width: `${Math.max(0, Math.min((metrics.usageMb % 1000) / 10, 100))}%`,
                  }}
                />
              </div>
            </div>
              ...
              >
                <div
                  style={{
                    ...
                    background: "var(--accent)",
                    borderRadius: 999,
                    width: `${Math.max(0, Math.min((metrics.sessionCount % 1000) / 10, 100))}%`,
                  }}
                />
              </div>
            </div>
  ...
  function renderAccounts() {
    ...
    const suspendOwner = async (ownerId, suspended) => {
      try {
        const res = await fetch(`/api/super/owners/${ownerId}/suspend`, {
          method: "POST",
          headers,
    ...
    const deleteOwner = async (ownerId) => {
      try {
        ...
        const res = await fetch(`/api/super/owners/${ownerId}`, {
          method: "DELETE",
          headers,
        });
        const data = await res.json();
    ...
    return (
      <>
        <div className="yaba-card" style={{ marginBottom: 24 }}>
          ...
          >
            <h3 className="yaba-card-title" style={{ margin: 0 }}>
              Accounts
            </h3>
            <input
              placeholder="Search name, email, hotspot ID…"
          ...
          <div>
            ...
            >
              ...
              <input
                ...
                onChange={(e) =>
                  setNewOwner({ ...newOwner, ownerPhone: e.target.value })
                }
              />
              <input
                ...
                onChange={(e) =>
                  setNewOwner({ ...newOwner, ownerEmail: e.target.value })
                }
              />
              <input
                ...
                onChange={(e) =>
                  setNewOwner({ ...newOwner, businessName: e.target.value })
                }
              />
              <button
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <button className="yaba-btn yaba-btn--secondary">
              Owners ({qOwners.length})
            </button>
            <button className="yaba-btn yaba-btn--secondary">
              Users ({qUsers.length})
            </button>
              ...
              <div>{o.suspended ? "Suspended" : "Active"}</div>
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <button
                    className="yaba-btn yaba-btn--secondary"
                    style={{ padding: "6px 10px", fontSize: 12 }}
                    onClick={() => suspendOwner(o.id, !o.suspended)}
                  >
        ...
        {selectedOwner && (
          <div className="yaba-card">
            <h3 className="yaba-card-title">
              Sites for {selectedOwner.name || selectedOwner.ownerName}
            </h3>
            <div
              style={{
                display: "flex",
            ...
            >
              <input
                ...
                className="yaba-input"
                value={newSite.hotspotName}
                onChange={(e) =>
                  setNewSite({ ...newSite, hotspotName: e.target.value })
                }
              />
              <input
                placeholder="Address"
              ...
              <input
                ...
                value={newSite.ssid}
                onChange={(e) =>
                  setNewSite({ ...newSite, ssid: e.target.value })
                }
              />
              <select
                className="yaba-select"
                value={newSite.routerType}
                onChange={(e) =>
                  setNewSite({ ...newSite, routerType: e.target.value })
                }
              >
                ...
                try {
                  const token = localStorage.getItem("superToken");
                    const headers = token
                      ...
                      : { "Content-Type": "application/json" };
                    const res = await fetch(
                      `/api/super/sites/${selectedOwner.id}`,
                      {
                        method: "POST",
                        headers,
            ...
            {defaultPackages.map((p, idx) => (
              ...
              >
                <input
                  value={p.name}
                  onChange={(e) => {
                    const next = [...defaultPackages];
                    next[idx] = { ...p, name: e.target.value };
                    setDefaultPackages(next);
                  }}
                  className="yaba-input"
                  style={{ flex: 2 }}
                />
        ...
        <div className="yaba-card">
          ...
          >
            ...
            <input
              ...
              className="yaba-input"
              style={{ width: 120 }}
              value={businessRules.taxPercent}
              onChange={(e) =>
                setBusinessRules({
                  ...businessRules,
            ...
            <select
              className="yaba-select"
              style={{ width: 200 }}
              value={businessRules.paymentProvider}
              onChange={(e) =>
                setBusinessRules({
                  ...businessRules,
  ...
  function renderWallet() {
    ...
    const savePayout = async () => {
      try {
        const res = await fetch("/api/super/payout-account", {
          method: "POST",
          headers,
        ...
        <div style={styles.gridTwo}>
          <div style={styles.widget}>
            ...
            <div style={{ fontSize: 28, fontWeight: 700 }}>
              UGX {Number(wallet.balance).toLocaleString()}
            </div>
          ...
          <div style={styles.widget}>
            ...
            <div style={styles.formRowInline}>
              ...
              <select
                value={payoutAccount.accountType}
                onChange={(e) =>
                  setPayoutAccount({
                    ...payoutAccount,
            ...
            <button style={styles.primaryBtnSmall} onClick={savePayout}>
              Save
            </button>
          </div>
        </div>
        ...
        >
                ...
                (reports.transfers || []).slice(0, 10).map((t, i) => (
                  ...
                  >
                    ...
                    <div style={{ color: "var(--text-muted)", fontSize: 12 }}>
                      {new Date(t.settledAt).toLocaleString()}
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="yaba-card">
            ...
            >
              ...
              <a
                href="/api/super/reports/export/csv"
                className="yaba-btn yaba-btn--secondary"
                style={{
                  padding: "8px 16px",
                fontSize: 12,
              ...
              {(reports.splits || []).length === 0 ? (
                ...
                (reports.splits || []).slice(0, 20).map((s, i) => (
                  ...
                  >
                    ...
                    <div style={{ color: "var(--text-muted)", fontSize: 12 }}>
                      Gross: UGX {s.grossAmount}
                    </div>
                    <div style={{ color: "var(--accent)", fontSize: 12 }}>
                      Comm: UGX {s.commissionAmount}
                    </div>
  ...
  function renderSupport() {
    ...
    const saveContact = async () => {
      try {
        ...
        alert(
          "Support contact info saved. This will reflect on the Admin Login page.",
        );
      } catch (e) {
  ...
  function renderSettings() {
    ...
    const saveAlerts = async () => {
      try {
        const res = await fetch("/api/super/alerts-settings", {
          method: "POST",
          headers,
        ...
        alert("Failed to save alerts settings");
      }
    };
      ...
      <div style={styles.widget}>
        <h3 style={styles.widgetTitle}>Platform Settings</h3>
          <div style={styles.formRow}>
            ...
            <input style={styles.input} defaultValue="WiFi Platform" />
          </div>
          <div style={styles.formRow}>
            <label>Support Email</label>
            <input style={styles.input} defaultValue="support@example.com" />
          </div>
          <div style={styles.formRow}>
            <label>Admin Contact Phone</label>
            <input style={styles.input} defaultValue="+256700000000" />
          </div>
        ...
        <div style={styles.widget}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                ...
                onKeyPress={(e) => {
                  if (e.key === "Enter" && e.target.value.trim()) {
                    setAlertsSettings((prev) => ({
                      ...prev,
                      emails: [...prev.emails, e.target.value.trim()],
                    }));
                    e.target.value = "";
                  }
                }}
              ...
              <button
                ...
                if (input?.value.trim()) {
                    setAlertsSettings((prev) => ({
                      ...prev,
                      emails: [...prev.emails, input.value.trim()],
                    }));
                    input.value = "";
                  }
                }}
          ...
          <div style={styles.formRow}>
            ...
            <input
              type="number"
              style={styles.input}
              value={alertsSettings.thresholds.downHotspotMinutes}
              onChange={(e) =>
                setAlertsSettings((prev) => ({
                  ...prev,
                  thresholds: {
                    ...prev.thresholds,
                    downHotspotMinutes: Number(e.target.value),
                  },
                }))
              }
            />
          </div>
          <button style={styles.primaryBtnSmall} onClick={saveAlerts}>
            Save Alerts
          </button>
        </div>
      </>
      ...
      <div style={styles.widget}>
        ...
        <div style={{ ...styles.table }}>
          ...
          {logs.slice(0, 50).map((log, i) => (
            <div
              key={i}
              style={{ ...styles.tr, gridTemplateColumns: "1fr 1fr 1fr 1fr" }}
            >
              <div>{log.actor || "—"}</div>
              <div>{log.action || "—"}</div>
              <div>{log.ownerId || "—"}</div>
  ...
  function renderOwnerDetails() {
    ...
    const { owner, stats, sites, routers, packages, recentSessions } =
      ownerDetails;
      ...
      <>
        <div style={styles.widget}>
          <h3 style={styles.widgetTitle}>
            Owner Details: {owner.ownerName || "Unnamed"}
          </h3>
          <div
            style={{
              display: "grid",
          ...
          >
            ...
            <div style={styles.statCard}>
              ...
              <div style={styles.statValue}>
                UGX {Number(stats.totalRevenue).toLocaleString()}
              </div>
            </div>
            <div style={styles.statCard}>
              <div style={styles.statLabel}>Commission</div>
              <div style={styles.statValue}>
                UGX {Number(stats.totalCommission).toLocaleString()}
              </div>
            </div>
          </div>
          <div style={styles.formRowInline}>
            <span>
              <strong>Email:</strong> {owner.ownerEmail || "—"}
            </span>
            <span>
              ...
              {sites.map((site, i) => (
                ...
                >
                  ...
                  <div style={{ fontSize: 12 }}>
                    SSID: {site.ssid || "—"} | Status: {site.status}
                  </div>
              </div>
              ))}
            </div>
            ...
            <div style={{ maxHeight: 300, overflow: "auto" }}>
              {packages.map((pkg, i) => (
                ...
                >
                  ...
                  <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
                    UGX {Number(pkg.price || 0).toLocaleString()}
                  </div>
                  <div style={{ fontSize: 12 }}>
                    Status: {pkg.status || "—"}
                  </div>
        ...
        <button
          style={{ ...styles.secondaryBtnSmall, marginTop: 16 }}
  ...
  function renderHealthMonitoring() {
    if (!healthData)
      return (
        <div style={styles.widget}>
          <h3 style={styles.widgetTitle}>Loading health data...</h3>
        </div>
      );
    ...
    return (
      <>
        <div style={styles.statsGrid}>
          ...
          <Stat
            label="Revenue (24h)"
            value={healthData.metrics.revenueLast24h}
          />
        </div>
        ...
        <div style={styles.gridTwo}>
          ...
          <div style={styles.widget}>
            <h3 style={styles.widgetTitle}>
              Active Alerts ({healthData.metrics.alertsCount})
            </h3>
            <div style={{ maxHeight: 300, overflow: "auto" }}>
          ...
          >
            ...
            {/* Platform Metrics */}
              ...
              <div style={{ textAlign: "right", minWidth: "80px" }}>
                ...
                >
                  {accounts.owners?.filter((o) => !o.suspended).length || 0}/
                  {accounts.owners?.length || 0}
            </div>
              </div>
              <div style={{ textAlign: "right", minWidth: "80px" }}>
                <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
                  ...
                  createPortal(
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {["all", "system", "alerts", "owners", "sites"].map(
                            (f) => (
                              <button
                                key={f}
