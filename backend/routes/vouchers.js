...
router.get("/owner/:ownerId/export/csv", async (req, res) => {
  try {
    ...
    const snapshot = await db
      ...
      .get();
    const rows = [
      "code,type,value,usageCount,usageLimit,status,createdAt,expiresAt,redeemedAt,redeemedByPhone",
    ];
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=vouchers_${ownerId}.csv`,
    );
    res.send(rows.join("\n"));
  } catch (e) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to export vouchers" });
  }
});
...
router.get("/owner/:ownerId/export/pdf", async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=vouchers_${ownerId}.pdf`,
    );
    doc.pipe(res);
    ...
    const cols = 3;
    const colWidth =
      (doc.page.width - doc.page.margins.left - doc.page.margins.right) / cols;
    let col = 0;
    let rowY = doc.y;
    ...
    for (const v of vouchers) {
      ...
      doc
        .fontSize(10)
        .text(
          `Type: ${v.type === "free_access" ? "Free Access" : "Discount"}`,
          x + 10,
          y + 28,
        );
      doc
        .fontSize(10)
        .text(
          `Value: ${v.packageValue}${v.type === "free_access" ? " min" : "%"}`,
          x + 10,
          y + 44,
        );
      doc
      ...
      doc.image(Buffer.from(qrBase64, "base64"), x + boxW - 110, y + 10, {
        width: 100,
        height: 100,
      });
      ...
      if (rowY + boxH + 16 > doc.page.height - doc.page.margins.bottom) {
        doc.addPage();
  ...
  try {
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (
        ...
        voucherData.usageCount >= voucherData.usageLimit
      ) {
      ...
      const receiptRef = transaction.set(
    ...
    try {
      await createWiFiSessionFromVoucher(
        phone,
        result.packageName,
        result.ownerId,
        result.receiptId,
      ...
      console.warn(
        "WiFi session creation from voucher failed (non-blocking):",
        err.message,
      );
...
async function createWiFiSessionFromVoucher(
  phone,
  packageName,
  ownerId,
  receiptId,
  ...
  try {
    ...
    if (!hotspotsSnap.empty) {
      ...
      hotspotName =
        hotspotData.hotspotName || hotspotData.name || "WiFi Hotspot";
    }
    ...
    console.log(
      `WiFi session created from voucher for ${phone} at ${hotspotName}`,
    );
  } catch (error) {
...
router.get("/owner/:ownerId/stream", async (req, res) => {
  try {
        ...
        (snapshot) => {
          ...
          const stats = {
            ...
            redeemedVouchers: vouchers.filter((v) => v.status === "redeemed")
              .length,
            expiredVouchers: vouchers.filter((v) => v.status === "expired")
              .length,
            totalRedemptions: vouchers.reduce(
              (sum, v) => sum + (v.usageCount || 0),
              0,
    ...
    try {
      res.write(
        `data: ${JSON.stringify({ success: false, error: "init_failed" })}\n\n`,
      );
      res.end();
    } catch {}
  }
...
router.post("/apply", async (req, res) => {
  try {
    ...
    if (snap.empty)
      return res
        .status(404)
        .json({ success: false, message: "Invalid voucher code" });
    ...
    if (v.ownerId !== ownerId) {
      return res
        .status(403)
        .json({ success: false, message: "Voucher owner mismatch" });
    }
    if (v.status !== "active") {
      return res
        .status(400)
        .json({ success: false, message: `Voucher is ${v.status}` });
    }
    ...
    if (v.usageLimit && (v.usageCount || 0) >= v.usageLimit) {
      return res
        .status(400)
        .json({ success: false, message: "Voucher usage limit exceeded" });
    }
    if (v.packageName && v.packageName !== packageName) {
      return res.status(400).json({
        success: false,
        message: `Voucher only valid for ${v.packageName}`,
      });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to apply voucher" });
  }
});
