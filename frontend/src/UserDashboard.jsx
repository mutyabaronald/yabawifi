...
function UserDashboard() {
        ...
        navigator.geolocation.getCurrentPosition(async (pos) => {
          ...
          try {
            const res = await axios.get(
              `/api/hotspots/nearby?lat=${latitude}&lng=${longitude}&radiusKm=3`
            );
  ...
  useEffect(() => {
    ...
    // Check session status every 5 seconds for near real-time updates
  ...
  // Refresh immediately when page/tab becomes visible
  useEffect(() => {
    const onVisibility = () => {
    ...
    const fetchLoyaltyRewards = async () => {
      try {
        const response = await axios.get(
          `/api/packages/loyalty-rewards/${currentSession.hotspotId}`
        );
  ...
  // Keep owner branding (logo/name) in sync with the active hotspot in real time
  useEffect(() => {
    const loadOwnerBranding = async () => {
      if (!currentSession.isConnected || !currentSession.hotspotId) {
        setOwnerInfo({
          ownerName: "",
          ownerPhone: "",
      ...
      try {
        // Try fetching owner branding by owner/hotspot id
        const res = await axios.get(
          `/api/owners/logo/${currentSession.hotspotId}`
        );
        setOwnerInfo({
          ownerName: res.data?.ownerName || "",
          ownerPhone: res.data?.ownerPhone || "",
          ownerWhatsapp: res.data?.ownerWhatsapp || "",
          logoUrl: res.data?.logoUrl || "",
          motto: res.data?.motto || res.data?.slogan || res.data?.tagline || "",
        });
        setOwnerId(currentSession.hotspotId);
      } catch (e1) {
        try {
          // Fallback: fetch via hotspot details then derive owner branding if provided
          const hs = await axios.get(
            `/api/hotspots/${currentSession.hotspotId}`
          );
          const branding = hs.data || {};
          setOwnerInfo({
            ownerName: branding.ownerName || branding.hotspotName || "",
            ownerPhone: branding.ownerPhone || "",
            ownerWhatsapp: branding.ownerWhatsapp || "",
            logoUrl: branding.logoUrl || "",
            motto: branding.motto || branding.slogan || branding.tagline || "",
          });
          setOwnerId(branding.ownerId || currentSession.hotspotId || "");
        } catch (e2) {
          // If both calls fail, keep existing branding but ensure hotspot name still shows
        }
      }
    };

    loadOwnerBranding();
  }, [currentSession.isConnected, currentSession.hotspotId]);
  ...
  const checkAndUpdateSessionStatus = async () => {
    try {
      ...
      const response = await axios.get(
        `/api/users/${userPhone}/active-packages`
      );
      ...
      if (activePackages.length > 0) {
        ...
        setCurrentSession({
          ...
          sessionProgress:
            activePackage.dataUsed && activePackage.dataLimit
              ? (activePackage.dataUsed / activePackage.dataLimit) * 100
              : 0,
        });
    ...
    try {
      ...
      const [transactionsRes, packagesRes, loyaltyRes] = await Promise.all([
        axios
          ...
          .catch(() => ({ data: [] })),
        axios
          .get(`/api/users/${phone}/active-packages`)
          .catch(() => ({ data: [] })),
        axios
        ...
        if (lastTransaction.ownerId) {
          try {
            const ownerRes = await axios.get(
              `/api/owners/logo/${lastTransaction.ownerId}`
            );
            setOwnerInfo({
              ...
              motto:
                ownerRes.data.motto ||
                ownerRes.data.slogan ||
                ownerRes.data.tagline ||
                "",
            });
            setOwnerId(lastTransaction.ownerId);
            try {
              const notifRes = await axios.get(`/api/owner/notifications`, {
                params: { ownerId: lastTransaction.ownerId },
              });
              const notifications = Array.isArray(notifRes.data?.notifications)
  ...
  const handleRedeemPoints = async (e) => {
    ...
    const pointsRequired =
      redeemPackage === "1hour" ? 100 : redeemPackage === "1day" ? 500 : 1000;

    if (loyaltyPoints < pointsRequired) {
      setRedeemMessage(
        `You need ${pointsRequired} points to redeem this package. You have ${loyaltyPoints} points.`
      );
      return;
    }
    ...
    try {
      const response = await axios.post(
        `/api/users/${userPhone}/loyalty/redeem`,
        {
      ...
      if (response.data.success) {
        setRedeemMessage(
          "✅ Points redeemed successfully! Your free access has been activated."
        );
  ...
  const handleLoyaltyRedemption = async (rewardPackageId) => {
    if (
      ...
      setRedeemMessage(
        "❌ You must be connected to a WiFi hotspot to redeem loyalty points"
      );
      return;
    }
    ...
    try {
      const response = await axios.post(
        `/api/users/${userPhone}/loyalty/${currentSession.hotspotId}/redeem`,
        {
      ...
      const message =
        error.response?.data?.error || "Failed to redeem loyalty points";
      setRedeemMessage(`❌ ${message}`);
    } finally {
  ...
  return (
    <div
      style={{
        ...
        backgroundColor: isDarkMode ? "var(--n13)" : "var(--n3)",
      }}
    >
      {!isDesktop && !sidebarOpen && (
        <button style={styles.openFab} onClick={() => setSidebarOpen(true)}>
          ☰
        </button>
      )}
      <aside
        ...
        <div style={styles.sidebarInner}>
                  ...
                  {ownerInfo?.logoUrl ? (
                    ...
                    <div
                      style={{
                        ...
                        background: isDarkMode ? "var(--n13)" : "var(--n5)",
                        color: isDarkMode ? "var(--n8)" : "var(--n11)",
                        fontWeight: 700,
                      }}
                    >
                      {currentSession.hotspotName
                        ? currentSession.hotspotName.charAt(0)
                        : ""}
                    </div>
                  )}
          ...
          >
            <div style={styles.profilePictureSidebar}>
              <span style={styles.profileInitialSidebar}>
                {userPhone.charAt(0)}
              </span>
              {profileNotifications > 0 && (
                <span style={styles.profileNotificationBadgeSidebar}>
          ...
          {(!isDesktop || sidebarExpanded) && (
            <div
              style={{
                ...styles.connectionStatusSidebar,
            ...
            >
              <div
                style={{
                  ...styles.statusDotSidebar,
                  backgroundColor: currentSession.isConnected
                    ? "#10b981"
                    : "#6b7280",
                }}
              ></div>
          ...
          <div
            style={{
              ...
              borderTop: isDarkMode ? "1px solid #111827" : "1px solid #e2e8f0",
            }}
          >
      ...
      >
        ...
        <div style={styles.topHeader}>
          <div style={styles.connectionStatus}>
            ...
            <span style={styles.statusText}>
              {currentSession.isConnected
                ? `Connected to ${currentSession.hotspotName}`
                : "Not connected yet"}
            </span>
          ...
          <div style={styles.headerRight}>
            ...
            <div style={styles.profileSection}>
              <div style={styles.profilePicture}>
                ...
                {profileNotifications > 0 && (
                  <span style={styles.profileNotificationBadge}>
                    {profileNotifications}
                  </span>
                )}
              </div>
        ...
        {activeTab === "transactions" && (
          <div style={styles.section}>
            {console.log(
              "Rendering TransactionsSection with transactions:",
              transactions
            )}
            <TransactionsSection
        ...
        {activeTab === "loyalty" && (
          <div style={styles.section}>
            {console.log("Rendering LoyaltyProgram with props:", {
              userPhone,
              currentSession,
              ownerId,
            })}
          ...
          <div style={styles.dashboardContainer}>
            ...
            {console.log("Dashboard data:", {
              ...
              transactions: Array.isArray(transactions)
                ? transactions.length
                : "not array",
              activePackages: Array.isArray(activePackages)
            ...
            <div className="yaba-card" style={styles.welcomeBanner}>
              <div style={styles.welcomeText}>
                <h2 className="yaba-card-title">
                  Welcome back, {userPhone}! 👋
                </h2>
                <p className="yaba-muted">
                  {currentSession.isConnected
                    ? `You're connected to ${currentSession.hotspotName}`
                    : "Connect to a WiFi hotspot to get started"}
                </p>
              </div>
              ...
              {currentSession.isConnected && currentHotspotLoyalty > 0 && (
                <div
                  ...
                  style={styles.loyaltyPointsBanner}
                >
                  ⭐ {currentHotspotLoyalty} Loyalty Points at{" "}
                  {currentSession.hotspotName}
                </div>
              )}
            </div>
            ...
            <div style={styles.metricsGrid}>
              <div className="yaba-card">
                ...
                <div className="yaba-muted" style={{ fontSize: "12px" }}>
                  vs last month
                </div>
              </div>
            ...
            <div className="yaba-card" style={styles.currentSessionSection}>
                    ...
                    >
                      {currentSession.isConnected
                        ? `${formatDataUsage(
                            ...
                            currentSession.dataLimit
                          )} (${currentSession.sessionProgress.toFixed(1)}%)`
                        : "0 MB / 0 MB (0%)"}
                    </div>
                  </div>
                </div>
                ...
                {activePackages.map((pkg, index) => (
                  ...
                  >
                    ...
                    <div style={styles.packageDetails}>
                      <p className="yaba-muted">
                        Data/Time Remaining: {pkg.remaining || "Unlimited"}
                      </p>
                      <p className="yaba-muted">
          ...
          <div style={styles.modalOverlay}>
            <div className="yaba-card" style={styles.modal}>
              ...
              <div style={styles.modalContent}>
                ...
                <div
                  className="yaba-card yaba-elev-2"
                  style={styles.currentPointsSection}
                >
                  ...
                  <div style={styles.loyaltyRewardsSection}>
                    ...
                    <div style={styles.rewardsGrid}>
                      {availableLoyaltyRewards.map((reward) => {
                        const canAfford =
                          currentHotspotLoyalty >= reward.pointsRequired;
                          ...
                          >
                            ...
                            <div style={styles.rewardDetails}>
                              {reward.rewardType === "time_access" && (
                                <span className="yaba-muted">
                                  ⏰ {reward.rewardValue} minutes
                                </span>
                              )}
                              {reward.rewardType === "data_access" && (
                                <span className="yaba-muted">
                                  📊 {reward.rewardValue} MB
                                </span>
                              )}
                ...
                {redeemMessage && (
                  <div
                    className="yaba-card yaba-elev-2"
                    style={{
                      backgroundColor: redeemMessage.startsWith("✅")
                        ? "var(--success)"
                        : "var(--danger)",
                      color: "white",
                      marginTop: "16px",
