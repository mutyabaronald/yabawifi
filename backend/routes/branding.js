...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    cb(
      null,
      `${req.body.ownerId}-${req.body.hotspotId}-${
        file.fieldname
...
const upload = multer({
  ...
  fileFilter: function (req, file, cb) {
    ...
    if (
      ...
      cb(
        new Error(
          `Invalid file type for ${fileType}. Allowed types: ${allowedTypes[
            fileType
          ].join(", ")}`
        )
      );
...
router.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    const brandingRef = db
      .collection("branding")
      .doc(`${ownerId}_${hotspotId}`);
...
router.post("/save", async (req, res) => {
  try {
    ...
    if (!ownerId || !hotspotId) {
      return res
        .status(400)
        .json({ success: false, error: "Missing ownerId or hotspotId" });
    }
...
router.post("/publish", async (req, res) => {
  try {
    ...
    const liveBrandingRef = db
      .collection("live_branding")
      .doc(`${ownerId}_${hotspotId}`);
...
router.get("/:ownerId/:hotspotId", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch branding data" });
  }
});
...
router.get("/live/:ownerId/:hotspotId", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch live branding data" });
  }
});
